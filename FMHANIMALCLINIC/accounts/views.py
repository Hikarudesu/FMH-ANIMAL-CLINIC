...
def vet_dashboard_view(request):
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # VET'S PATIENT UNIVERSE  (single queryset, reused everywhere)
    # ═══════════════════════════════════════════════════════════════════════
    # A pet belongs to this vet if:
    #   – it has at least one Appointment with preferred_vet = this vet, OR
    #   – it has at least one MedicalRecord written by this vet.
    # Using a Q-union avoids materialising IDs into Python sets.

    if staff_profile:
        vet_pet_qs = Pet.objects.filter(
            Q(appointments__preferred_vet=staff_profile)
            | Q(medical_records__vet=staff_profile)
            | Q(owner__branch=staff_profile.branch)
            | Q(owner__isnull=True, source='WALKIN')
        ).distinct()
    else:
    ...
    if staff_profile:
        medical_records_count = RecordEntry.objects.filter(
            Q(vet=staff_profile) | Q(record__branch=staff_profile.branch, vet__isnull=True)
        ).count()
    else:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # Groups ALL clinic pets by their current clinical_status FK.
    # Showing all pets ensures newly registered pets reflect immediately on the dashboard.

    patient_status_distribution = []
    if staff_profile:
        status_counts = {}
        for pet in Pet.objects.select_related('clinical_status').iterator():
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # All-time breakdown of appointment reasons for this vet.
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # CHART 3 — WEEKLY WORKLOAD  (bar chart, Sun–Sat)
    # ═══════════════════════════════════════════════════════════════════════
    # Index 0 = Sunday … 6 = Saturday, matching JS `new Date().getDay()`.

    weekly_workload = [0] * 7
    if staff_profile:
        # Sunday-aligned start of the current week
        py_dow = today.weekday()                   # Mon=0 … Sun=6
        days_since_sunday = (py_dow + 1) % 7
        start_of_week = today - timedelta(days=days_since_sunday)
        end_of_week = start_of_week + timedelta(days=6)

        # Single aggregate query for the whole week (including branch walk-ins & future limits)
        week_daily = (
            Appointment.objects.filter(
                pet__isnull=False,
            ...
            .filter(Q(preferred_vet=staff_profile) | Q(branch=staff_profile.branch, preferred_vet__isnull=True))
            .exclude(status='CANCELLED')
            .values('appointment_date')
            .annotate(cnt=Count('id'))
        )
        for row in week_daily:
            # Convert DB date → Sun-based index
            d = row['appointment_date']
            idx = (d.weekday() + 1) % 7             # Sun=0 … Sat=6
    ...
    if staff_profile:
        last_30_appts = Appointment.objects.filter(
            Q(preferred_vet=staff_profile) | Q(branch=staff_profile.branch, preferred_vet__isnull=True),
    ...
    if staff_profile:
        # Count pets strictly registered this month belonging to the branch's universe
        new_patients_this_month = vet_pet_qs.filter(
            created_at__range=[month_start, today + timedelta(days=1)]
        ).count()

        # Total unique patients seen this month (new + returning combined, scheduled/arrived/completed)
    ...
    # so that pending/confirmed appointments are also counted.
    ...
    if staff_profile:
        ...
        _vet_q = Q(preferred_vet=staff_profile) | Q(branch=staff_profile.branch, preferred_vet__isnull=True)
        ...
        _month_appts_completed = Appointment.objects.filter(
            _vet_q, status='COMPLETED', **_base_filter
        )
    ...
    # ── Monthly Summary ──────────────────────────────────────────────────
    if staff_profile:
        medical_records_this_month = RecordEntry.objects.filter(
            Q(vet=staff_profile) | Q(record__branch=staff_profile.branch, vet__isnull=True),
        ...
        cancelled_this_month = Appointment.objects.filter(
            Q(preferred_vet=staff_profile) | Q(branch=staff_profile.branch),
            appointment_date__range=[month_start, month_end],
            status='CANCELLED',
            pet__isnull=False,
        ).distinct().count()
...
def receptionist_dashboard_view(request):
    ...
    # Branch filter - if user has a branch, filter by it; otherwise show all
    ...
    payment_filter = {'sale__branch': user_branch} if user_branch else {}
    ...
    # ─── LOW STOCK PRODUCTS (branch-specific) ───
    low_stock_products = Product.objects.filter(
        is_available=True,
        stock_quantity__lte=F('min_stock_level'),
    ...
    for hour in range(8, 20):  # 8 AM to 8 PM
        ...
        hourly_sales_data.append({
            ...
            'total': float(hour_sales['total']) if hour_sales['total'] else 0
        })
